...
public final class MainActivity extends Activity {
    ...
    private static final int REQ_FYERS_LOGIN = 45;
    private final AtomicBoolean fyersExchangeInFlight = new AtomicBoolean(false);
    ...
    private void build(){
        ...
        root.addView(t("NIFTY Microstructure Evidence — Android V0.3",22,true));
        ...
        root.addView(button("1. FYERS LOGIN INSIDE APP — AUTO CALLBACK",v->openFyersEmbeddedLogin()));
        root.addView(button("EXTERNAL BROWSER — MANUAL FALLBACK",v->openFyersExternalLogin()));
        root.addView(fyersAuthCode);
        root.addView(button("MANUAL FALLBACK: EXCHANGE CALLBACK/CODE",v->exchangeFyers()));
        ...
        root.addView(t("FYERS OAuth V0.3 intercepts the exact registered HTTPS redirect inside the app, validates fresh random state, and exchanges the one-time auth_code immediately. External browser/manual copy is fallback only. TBT follows SDK 3.1.15 mechanics.",12,false));
    ...
    private void openFyersEmbeddedLogin(){
        try{
            save();
            String app=fyersApp.getText().toString().trim(),redirect=fyersRedirect.getText().toString().trim();
            String state=FyersOAuth.newState();
            String url=new BrokerAuthClient().buildFyersLoginUrl(app,redirect,state);
            fyersAuthCode.setText("");
            Intent i=new Intent(this,FyersLoginActivity.class)
                    .putExtra(FyersLoginActivity.EXTRA_LOGIN_URL,url)
                    .putExtra(FyersLoginActivity.EXTRA_REDIRECT_URI,redirect)
                    .putExtra(FyersLoginActivity.EXTRA_EXPECTED_STATE,state);
            setAuth("FYERS: secure in-app login opened. Callback will be intercepted before localhost navigation.");
            startActivityForResult(i,REQ_FYERS_LOGIN);
        }catch(Exception e){setAuth("FYERS LOGIN BLOCKED: "+safe(e.getMessage()));}
    }

    private void openFyersExternalLogin(){
        try{
            save();
            String state=FyersOAuth.newState();
            String url=new BrokerAuthClient().buildFyersLoginUrl(fyersApp.getText().toString(),fyersRedirect.getText().toString(),state);
            startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse(url)));
            setAuth("FYERS manual fallback opened. Browser may show localhost connection refused; copy the COMPLETE final URL and paste it below. Generate a fresh code for each exchange.");
        }catch(Exception e){setAuth("FYERS LOGIN BLOCKED: "+safe(e.getMessage()));}
    }

    private void exchangeFyers(){
        exchangeFyersValue(fyersAuthCode.getText().toString().trim());
    }

    private void exchangeFyersValue(String codeOrUrl){
        String app=fyersApp.getText().toString().trim(),secret=fyersSecret.getText().toString().trim();
        if(!fyersExchangeInFlight.compareAndSet(false,true)){setAuth("FYERS: token exchange already in progress");return;}
        setAuth("FYERS: exchanging fresh one-time auth_code...");
        net.execute(()->{
            try{
                ...
                runOnUiThread(()->{String m=safe(e.getMessage());if(m.contains("-437"))m+=" | auth_code is one-time: generate a NEW login code; never retry the same code";setAuth("FYERS BLOCKED: "+m);});
            }finally{fyersExchangeInFlight.set(false);}
        });
    }
    ...
    @Override protected void onActivityResult(int req,int res,Intent data){
        ...
        if(req==REQ_FYERS_LOGIN){
            if(res==RESULT_OK&&data!=null){String code=data.getStringExtra(FyersLoginActivity.EXTRA_AUTH_CODE);if(code!=null&&!code.isBlank()){setAuth("FYERS: callback captured + state verified; exchanging immediately...");exchangeFyersValue(code);}else setAuth("FYERS BLOCKED: callback returned without auth_code");}
            else {String err=data==null?"login cancelled":data.getStringExtra(FyersLoginActivity.EXTRA_ERROR);setAuth("FYERS LOGIN BLOCKED: "+safe(err));}
            return;
        }
        if(req==44&&res==RESULT_OK&&data!=null){Uri u=data.getData();try(OutputStream o=getContentResolver().openOutputStream(u)){EvidenceExporter.zip(new File(exportPath),o);toast("Evidence ZIP exported");}catch(Exception e){toast("Export failed: "+e.getMessage());}}
    }
